...
const Sidebar = () => {
  ...
  return (
    <nav className="w-64 bg-[var(--background)] border-r border-[var(--border-color)] fixed h-full flex flex-col font-montserrat">
          ...
          {filters.map((filter) => (
            ...
            >
              {React.cloneElement(filter.icon, {
                className: `w-5 h-5 ${selectedFilter === filter.id ? 'text-[var(--primary-color)]' : 'text-[var(--text-secondary)]'}`
              })}
              <span className={`text-sm ${selectedFilter === filter.id ? 'font-semibold' : 'font-medium'}`}>
                {filter.label}
              </span>
            </button>
          ))}
